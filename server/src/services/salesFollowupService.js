    ...
    return {
      ...
      orders: orders.filter((order) => textValue(order.fields?.[orderFields.confirmStatus]) === '已入账')
        .map((order) => ({
          ...
          fulfillment_status: textValue(order.fields?.[orderFields.fulfillmentStatus]) || '未交付',
          details: details.filter((detail) => linkedRecordIds(detail.fields?.[detailFields.salesEntry]).includes(order.record_id))
            .map((detail) => ({
          ...
          payments: payments.filter((payment) => linkedRecordIds(payment.fields?.[paymentFields.salesEntry]).includes(order.record_id))
            .map((payment) => ({
